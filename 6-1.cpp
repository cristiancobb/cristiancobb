...
const GLchar * vertexShaderSource = GLSL(330,
		layout (location = 0) in vec3 postion; // Vertex data from Vertex Attrib Pointer 0
...
void URenderGraphics(void)
{
	...
	GLint projLoc = glGetUniformLocation(shaderProgram, "projectoin");
	...
	glutSwapBUffer(); // Flips the back buffer with the front buffer every frame. Similar to GL FLush

}

// Create the Shader progran
void UCreateShader()
{
	...
	glCompileShader(fragmentShader); // Complies the Fragment shader

	// Shader program
	shaderProgram = glCreateProgram(); // Creates the Shader program and returns and id
	glAttachShader(shaderProgram, vertexShader); // Attach Vertex shader to the Shader program
	glAttachShader(shaderProgram); //Link Vertex and Fragment shaders to Shader program
